...
//     setLoading(true);
//     try {
//       let url = `${process.env.REACT_APP_POLICY_BACKEND}admin/get-new-users?page=${page}&rows=${rows}`;
...
//         `${process.env.REACT_APP_POLICY_BACKEND}admin/get-new-users?page=${page}&rows=${rows}&search=${searchValue}`, "GET", 1, {});
//       const data1 = await response1.json();
//       setPsgList(data1.data);

//       const countResponse1 = await customFetchWithAuth(
//         `${process.env.REACT_APP_POLICY_BACKEND}admin/get-new-users-count?search=${searchValue}`, "GET", 1, {},);
...
//     setSelectedDocument(row.id);
//     setSelectedRow(row);
//     navigate(`/admin/create-user/${row.id}`, {
//       state: { id: row.id, emp_id: row.emp_id, emp_name: row.emp_name, fromHandleRowClick: true }
//     });
//   };
